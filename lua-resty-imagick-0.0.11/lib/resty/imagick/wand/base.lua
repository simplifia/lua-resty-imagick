...
_M.get_resolution = function(self, x, y)
    return handle_result(self, lib.fset(self.wand, x, y))
end
...
_M.write_files = function(self, file)
    return handle_result(self, lib.MagickWriteImagesFile(self.wand, file))
end


_M.set_density = function(self, aDensity)
  local density = ffi.string(aDensity)
  self.drawingWand = ffi.gc(lib.NewDrawingWand(), lib.DestroyDrawingWand)
  -- local drawingWand = lib.AcquireDrawingWand(nil, self.wand)
  local a=handle_result(self, lib.DrawSetDensity(self.drawingWand, density))
  ngx.log(0, "AAAAAAAAAAAAAAAAAAAAAAAAAAAA0 ", a);
  ngx.log(0, "AAAAAAAAAAAAAAAAAAAAAAAAAAAA1 ", ffi.string(lib.DrawGetDensity(self.drawingWand)));
  handle_result(self, lib.MagickDrawImage(self.wand, self.drawingWand))
    return a;
end


_M.get_density = function(self)
  local drawingWand = ffi.gc(lib.NewDrawingWand(), lib.DestroyDrawingWand)
  local b = ffi.string(lib.DrawGetDensity(self.drawingWand))
  ngx.log(0, "AAAAAAAAAAAAAAAAAAAAaffAAAAAAAA ", b )
    return b;
end


_M.draw2 = function(self)
  return handle_result(self, lib.MagickDrawImage(self.wand, self.drawingWand))
end

-- ex "PixelsPerInchResolution" see data
_M.set_image_units = function(self, resolution_type)
return handle_result(self, lib.MagickSetImageUnits(self.wand, wand_data.resolution_type():to_int(resolution_type)))
end

_M.get_image_units = function(self, resolution_type)
   local a=  lib.MagickGetImageUnits(self.wand)
   return a
end
...
--[[
_M.MagickGetResolution = function(self)
  local x_resolution, y_resolution = lib.MagickGetResolution(self.wand)
end]]


_M.set_image_resolution = function(self, x_resolution, y_resolution)
